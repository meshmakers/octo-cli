...
internal class LogInClientCredentialsCommand : Command<OctoToolOptions>
{
    ...
    public override CommandDocumentation? GetDocumentation() =>
        new(
            Samples:
            [
                new CodeSample(arguments: [], description: "Headless run ΓÇö export credentials, then login"),
                new CodeSample(arguments: [
                    new CodeSampleArgument(_clientIdArg, "my-script-client"),
                    new CodeSampleArgument(_clientSecretArg, "<secret>"),
                ],
                    description: "Or using shell args directly (args take precedence over env vars)"),
            ],
            Notes:
            [
