...
internal class GetPoolCommand : ServiceClientOctoCommand<ICommunicationServicesClient>
{
    private readonly IConsoleService _consoleService;
    private readonly IArgument _idArg;
    private readonly IArgument _jsonArg;

    public GetPoolCommand(ILogger<GetPoolCommand> logger, IOptions<OctoToolOptions> options,
        IConsoleService consoleService,
        ICommunicationServicesClient communicationServicesClient, IAuthenticationService authenticationService)
        : base(logger, Constants.CommunicationServicesGroup, "GetPool",
            "Gets the configuration for a specific pool.", options,
            communicationServicesClient, authenticationService)
    {
        _consoleService = consoleService;
        _idArg = CommandArgumentValue.AddArgument("id", "identifier", ["The pool runtime ID"], true, 1);
    ...
    public override async Task Execute()
    {
        ...
        Logger.LogInformation(
            "Getting pool '{PoolId}' for tenant '{TenantId}' at '{ServiceClientServiceUri}'",
        ...
        var result = await ServiceClient.GetPoolConfigurationAsync(id);
