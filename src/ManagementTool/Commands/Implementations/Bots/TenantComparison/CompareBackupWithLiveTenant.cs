...
internal class CompareBackupWithLiveTenant : JobWithWaitOctoCommand
{
    ...
    public CompareBackupWithLiveTenant(ILogger<CompareBackupWithLiveTenant> logger, IOptions<OctoToolOptions> options,
        IBotServicesClient botServicesClient, IAuthenticationService authenticationService)
        : base(logger, Constants.BotServicesGroup, "CompareBackupWithLiveTenant",
            "Compares a backup archive with a live tenant", options, botServicesClient, authenticationService)
    {
        _backupFileArg = CommandArgumentValue.AddArgument("sf", "sourceBackupFile",
            ["Backup file path (*.tar.gz)"], true, 1);
        _tenantIdArg = CommandArgumentValue.AddArgument("ttid", "targetTenantId",
    ...
    public override async Task Execute()
    {
        ...
        var targetTenantId = CommandArgumentValue.GetArgumentScalarValue<string>(_tenantIdArg).ToLower();
        ...
        Logger.LogInformation(
            "Starting comparison of backup '{BackupFile}' with live tenant '{TenantId}' at '{ServiceClientServiceUri}'",
            backupFile, targetTenantId, ServiceClient.ServiceUri);
        ...
        var response =
            await ServiceClient.CompareBackupWithLiveTenantAsync(tenantId, backupFile, targetTenantId, optionsJson);
        Logger.LogInformation("Tenant comparison job with id '{JobId}' has been started", response.JobId);

        await WaitForJob(response.JobId);

        Logger.LogInformation(
            "Comparison of backup with tenant '{TenantId}' completed", targetTenantId);
