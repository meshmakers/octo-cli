...
public class GenerateOperatorCertificatesCommand : Command<OctoToolOptions>
{
    ...
    public GenerateOperatorCertificatesCommand(ILogger<GenerateOperatorCertificatesCommand> logger,
        ...
        : base(logger, Constants.DevOpsGroup, "GenerateOperatorCertificates",
            "Generate CA and service/server certificates to run OctoMesh operator", options)
    {
        _outputArg =
            CommandArgumentValue.AddArgument("o", "output", ["Directory path where certificates are stored."], true, 1);
        _serverNameArg = CommandArgumentValue.AddArgument("s", "serverName",
            ["Name of service or server of operator, e. g. octo-mesh-op1-communication-operator."], true, 1);
        _namespaceArg = CommandArgumentValue.AddArgument("n", "namespace",
