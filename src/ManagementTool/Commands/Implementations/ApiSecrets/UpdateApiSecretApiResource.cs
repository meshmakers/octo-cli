...
internal class UpdateApiSecretApiResource : ServiceClientOctoCommand<IIdentityServicesClient>
{
    ...
    public UpdateApiSecretApiResource(ILogger<UpdateApiSecretApiResource> logger, IOptions<OctoToolOptions> options,
        ...
        : base(logger, "UpdateApiSecretApiResource", "Updates an API secret for an API resource.", options, identityServicesClient, authenticationService)
    {
    ...
    public override async Task Execute()
    {
        ...
        Logger.LogInformation("Updating API secret \'{SecretValue}\' for API resource \'{Name}\' at \'{ServiceClientServiceUri}\'", secretValue,name,
            ServiceClient.ServiceUri);

        var secretDto = await ServiceClient.GetApiSecretForApiResource(name, secretValue);
        if (secretDto == null)
        {
            Logger.LogError("API secret \'{SecretValue}\' for API resource \'{Name}\' at \'{ServiceClientServiceUri}\' not found", secretValue,name,
                ServiceClient.ServiceUri);
            return;
        }
        ...
        Logger.LogInformation("API secret \'{SecretValue}\' for API resource \'{Name}\' at \'{ServiceClientServiceUri}\' updated", secretValue, name,
            ServiceClient.ServiceUri);
    }
}
