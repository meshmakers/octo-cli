...
$tempFolderName = 'SandboxTestChoco'
...
"@
Write-Host "Here $tempFolder"

if (-Not [String]::IsNullOrWhiteSpace($Manifest)) {
  $manifestFileName = Split-Path $Manifest -Leaf
  $tempPathInSandbox = Join-Path -Path $desktopInSandbox $tempFolderName
  $manifestPathInSandbox = Join-Path -Path $tempPathInSandbox $manifestFileName
...
Write-Host "Here $tempPathInSandbox"
choco install octo-cli --debug --verbose --source '$tempPathInSandbox' -y
